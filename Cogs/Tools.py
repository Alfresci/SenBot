...
>>>>>>> ee3dfc6382ff1dc9b3f4cfc72f2fe2b15094bc69
    ...
    @commands.has_any_role('Events Team', 'Support Team', 'Development Team', 'COO', 'COS', 'COM', 'SS')
...
>>>>>>> ee3dfc6382ff1dc9b3f4cfc72f2fe2b15094bc69
    ...
    @commands.has_any_role('Sr. Moderator', 'Operations Manager', 'COO', 'COS', 'COM', 'SS')
...
>>>>>>> ee3dfc6382ff1dc9b3f4cfc72f2fe2b15094bc69
    ...
    async def create(self, ctx, *, reason='No reason provided'):
        ...
        channel = await guild.create_text_channel(name=f'{ctx.author.name}', overwrites=overwrites, reason=reason,
                                                  topic=f'Support Ticket')
        await channel.send(f'{user.mention}')
        await asyncio.sleep(2)
        await channel.purge(limit=1)
        embed1 = nextcord.Embed(title='Reason:', description=f'{reason}')
        await channel.send(embed=embed1)
        embed = nextcord.Embed(title='Channel Created!', description=f'Your ticket has been created! {channel.mention}')
        await ctx.send(embed=embed)
        await asyncio.sleep(5)
    ...
    async def add(self, ctx, member: nextcord.Member):
        ...
        if channel.topic == 'Support Ticket':
            await channel.set_permissions(member, overwrite=overwrite)
            await ctx.send(f'{member.name} has been added to the ticket!')
        else:
